...
export default function AdminDashboard() {
  ...
  const [cardOrder, setCardOrder] = React.useState(() => {
    const saved = localStorage.getItem('admin_dashboard_card_order');
    ...
    if (saved) {
      ...
      if (!parsed.includes('notifications')) {
        ...
        localStorage.setItem('admin_dashboard_card_order', JSON.stringify(updated));
        return updated;
      }
      return parsed;
    }
  ...
  async function sendClockInReminder(employeeUserId, employeeName) {
    try {
      ...
      // Obtener el company_id del admin
  ...
  async function loadDashboardData() {
    try {
      ...
      if (user) {
        ...
        if (userRole) {
          setAdminId(userRole.id);
          setCompanyId(userRole.company_id);
          await Promise.all([
            loadTeamStats(userRole.company_id),
            loadTeamMembers(userRole.company_id),
            loadRecentRequests(userRole.company_id)
          ]);
        }
      }
    } catch (error) {
  ...
  async function loadTeamStats(companyId) {
    try {
      // Obtener TODOS los empleados de la empresa (admin ve todo)
      const { data: allEmployees, error: employeesError } = await supabase
        .from('user_company_roles')
        .select('user_id')
        .eq('company_id', companyId)
        .eq('is_active', true)
        .in('role', ['employee', 'manager']); // Excluir admin y owner
      ...
      // Presentes hoy (contar los que tienen fichajes hoy)
      const today = new Date();
      ...
      if (teamUserIds.length > 0) {
        ...
        if (todayEntries) {
          const uniqueUsers = new Set(todayEntries.map(e => e.user_id));
          presentToday = uniqueUsers.size;
        }
      }

      // Solicitudes pendientes de TODA la empresa
      ...
      // Horas del equipo (simulado por ahora)
  ...
  async function loadRecentRequests(companyId) {
    try {
      // Obtener TODOS los empleados de la empresa (admin ve todo)
      const { data: allEmployees, error: employeesError } = await supabase
        .from('user_company_roles')
  ...
  async function handleRequestAction(requestId, status, requestType = 'normal', comments = '') {
    try {
      ...
      if (companyId) {
        await loadRecentRequests(companyId);
      }
    } catch (error) {
  ...
  function handleDragEnd(event) {
    ...
    if (over && active.id !== over.id) {
      setCardOrder((items) => {
        ...
        localStorage.setItem('admin_dashboard_card_order', JSON.stringify(newOrder));
        return newOrder;
      });
    }
  }
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-muted-foreground mt-1 text-sm sm:text-base">
            Gestión y supervisión de toda la empresa
          </p>
        </div>
      </div>
        ...
        >
          ...
          <div className="border-b border-border flex-shrink-0 p-4 sm:p-6">
            <div className="flex items-center gap-2 md:gap-3">
              ...
              <h3 className="text-base md:text-lg font-semibold text-foreground">Todos los Empleados</h3>
            </div>
          </div>
          <div className="p-4 md:p-6 flex-1 overflow-y-auto">
            {teamMembers.length === 0 ? (
              <div className="text-center py-8">
                ...
                <p className="text-muted-foreground">No hay empleados en la empresa</p>
              </div>
            ) : (
                      ...
                      <div className="card w-full h-full min-h-[300px] lg:h-[304px] flex flex-col">
                        ...
                        <div className="p-4 md:p-6 flex-1">
                          <div className="text-center py-4 md:py-8">
                            <p className="text-muted-foreground text-sm md:text-base">No hay empleados en la empresa</p>
                          </div>
                        </div>
                      </div>
                    )}
        ...
        <div className="border-b border-border flex-shrink-0" style={{ padding: '24px' }}>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2 md:gap-3">
              ...
              <h3 className="text-base md:text-lg font-semibold text-foreground">Solicitudes de la empresa</h3>
            </div>
