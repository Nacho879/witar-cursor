...
export default function Team() {
  ...
  async function loadTeamData() {
    try {
      ...
      if (managerRole) {
        // Cargar miembros del equipo (empleados que tienen a este manager como supervisor)
        ...
        if (!error && members) {
          ...
          await loadTeamStats(members, managerRole.company_id);
        }
      }
    } catch (error) {
  ...
  function calculateTeamStats(members, timeEntries) {
    ...
    members.forEach(member => {
      const memberEntries = timeEntries.filter(entry => entry.user_id === member.user_profiles?.id);
      ...
      memberHours[member.user_profiles?.id] = memberHoursWorked;
              ...
              {filteredMembers.map((member) => (
                <div key={member.id} className="card p-6 hover:shadow-lg transition-shadow">
                  ...
                  <div className="mt-4 pt-4 border-t border-border">
                    <button
                      onClick={() => {
                        // Implementar vista de fichajes del empleado
                        alert('Vista de fichajes próximamente');
                      }}
