-- Script corregido para verificar y corregir el estado del período de prueba para ilopez@witar.es
...
SELECT 
  ...
  dias_desde_creacion,
  CASE 
    WHEN dias_desde_creacion < 14 THEN 'EN PERÍODO DE PRUEBA'
    WHEN dias_desde_creacion >= 14 THEN 'PERÍODO DE PRUEBA EXPIRADO'
    ELSE 'ESTADO DESCONOCIDO'
  END as estado_empresa,
  CASE 
    WHEN dias_desde_creacion < 14 THEN 14 - dias_desde_creacion
    ELSE 0
...
SELECT 
  ...
  EXTRACT(DAY FROM (NOW() - c.created_at)) as dias_desde_creacion,
  CASE 
    WHEN EXTRACT(DAY FROM (NOW() - c.created_at)) < 14 THEN 'EN PERÍODO DE PRUEBA - EMPLEADOS ILIMITADOS'
