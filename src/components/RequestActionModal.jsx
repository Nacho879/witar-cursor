          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            <div>
              <label className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1.5">
                {isApproval ? 'Puedes dejar un comentario para el empleado.' : 'Motivo del rechazo *'}
              </label>
              <textarea
                value={comments}
                ...
                rows={2}
                className="w-full px-2.5 py-1.5 text-sm border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent dark:bg-gray-700 dark:text-white resize-none"
            ...
            <div className="flex justify-end gap-2 pt-2">
              <button
                ...
                onClick={handleClose}
                disabled={loading}
                className="px-3 py-1.5 text-sm text-gray-700 dark:text-gray-300 bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 rounded-lg transition-colors disabled:opacity-50"
              >
                Cancelar
              </button>
              <button
              ...
              >
                {loading ? (
                  ...
                  <>
                    {isApproval ? <Check className="w-3.5 h-3.5" /> : <AlertTriangle className="w-3.5 h-3.5" />}
                    {isApproval ? 'Aceptar' : 'Rechazar'}
                  </>
                )}
              </button>
            </div>
